...
void HitPointGauge::Update(void)
{
	...
	m_HitPointRate = (float)m_NowHitPoint / m_MaxHitPoint; // 何割消失したか

	size.x = m_HitPointRate * m_MaxSize; // 大きさの計算
...
void HitPointGauge::SetVertex(D3DXVECTOR2 size, D3DXCOLOR color)
{
	...
	{
		Vtx[CountVertex].Color = color;// 頂点カラー
	}
