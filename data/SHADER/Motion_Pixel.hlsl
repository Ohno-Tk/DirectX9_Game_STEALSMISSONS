...
/*	float4 lightAmbient = float4(0.3f,0.3f,0.3f,1.0f);
	...
	float l = max(dot( invertex.normalW, -lightDirL), 0.0f);
	invertex.color = (float4((lightDiffuse * l).rgb ,1.0f) + lightAmbient) * MaterialDiffuse;
...
*/
	...
	float3 Attenuation = float3(0.9f, 0.1f, 0.0f);
	...
	for(int i = 0; i < LIGHT_NUM; i++)
	{
	    ...
	    //色計算
	    color[i] = (float4(lightDiffuse.rgb ,1.0f) + lightAmbient) * MaterialDiffuse;
